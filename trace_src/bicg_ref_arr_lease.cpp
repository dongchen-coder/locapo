...
void bicg_cpu_trace(double* A, double* r, double* s, double* p, double* q, unsigned int nx, int ny) {
    ...
    {
        s[i] = 0.0;
    	trackingLease(S_OFFSET + i, 0, 0,8130);
	}

    for (i = 0; i < NX; i++)
    {
        q[i] = 0.0;
		trackingLease(Q_OFFSET + i, 1, 1,5);
        for (j = 0; j < NY; j++)
        {
            s[j] = s[j] + r[i] * A[i * NY + j];
            q[i] = q[i] + A[i * NY + j] * p[j];
        	trackingLease(S_OFFSET + j, 2, 0,3);
			trackingLease(R_OFFSET + i, 3, 2,9);
			trackingLease(A_OFFSET + i * NY + j, 4, 3,3);
			trackingLease(S_OFFSET + j, 5, 0,8134);
			trackingLease(Q_OFFSET + i, 6, 1,3);
			trackingLease(A_OFFSET + i * NY + j, 7, 3,5);
			trackingLease(P_OFFSET + j, 8, 4,8137);
			trackingLease(Q_OFFSET + i, 9, 1,5);
		}
    }

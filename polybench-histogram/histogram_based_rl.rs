...
fn run() -> Result<(), RllError> {
    ...
    let filepath = args.nth(1).ok_or(RllError::BadArgs)?;
    let mut suffix_str = "_ref_arr.tr";
    let prefix_str = &filepath[0..filepath.len()-9].to_string();
    let trace_str = [prefix_str, suffix_str].join("");
    println!("{}",trace_str);
    let input_file = File::open(filepath)?;
    let trace_file = File::open(trace_str)?;

    let mut buf = BufReader::new(trace_file);


    let mut trace_length_from_file:f32 = 0_f32;
    let mut line = String::new();
    if let Ok(mut bytes) = buf.read_line(&mut line) {
        while bytes > 0 {
            // Do something with the line
            trace_length_from_file += 1_f32;
            line.clear();
    ...
    println!("trace_length_from_file is {} ",trace_length_from_file);
...
//    println!("{}",csv_output_path);
    ...
    reference_level_lease(trace_length_from_file,
                          reuse_times,
        &data_block_id_trace,
        &ref_id_trace,
        &arr_id_trace,
...
//    }
    ...
    while size_ind != sizes.len() {
        ...
        let mut trace_len = trace.len() as f32;
...
//                println!();
            ...
            trace_len = trace_length_from_file/10_f32;
            println!();
            println!("*** Get miss ratio from PPUC ***");
            println!("Total costs: {}", total_costs);
            println!("Total costs (block) : {}", total_costs as f32/trace_len);
            println!("Total hits {}", total_hits);
            let mut miss_ratio = 1_f64 - total_hits / trace_len as f64;
            // I change the code here
            // average_cache_size is KB granularity, each data block is 8B so NumOfDataBlock x 8B/1024B
            let mut average_cache_size = (total_cost as f64 / (trace_len as f64)) as f64 * 8_f64 /1024_f64;
